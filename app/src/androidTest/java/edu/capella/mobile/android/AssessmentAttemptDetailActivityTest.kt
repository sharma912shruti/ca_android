 ...
 *
 */
class AssessmentAttemptDetailActivityTest {
    ...
    fun check_title_is_threads() {
        Espresso.onView(ViewMatchers.withId(R.id.backHeaderTxt))
            .check(ViewAssertions.matches(ViewMatchers.withText("Assess...")))
    }
    ...
    {
        Thread.sleep(500)
        var activity = getCurrentActivity() as AssessmentAttemptDetailActivity

        Assert.assertNotNull(activity.courseId)
        Assert.assertNotNull(activity.attemptList)
        Assert.assertNotNull( activity.assessmentAndStatus)
    }

    @Test
    fun check_assessment_detail_clickable(){
