...
class FormattedViewActivityTest {
    ...
    fun initPreWork(){
       ...
       // onView( withId( R.id.classmatesLayout)).perform( scrollTo(), click())
        ...
        onView(withText("Formatted Thread Test")).perform(click())


        Thread.sleep(100)

        onView(withId(R.id.view_formatted_post)) .perform(  click())


    }

    @Test
    fun check_formatter_view_is_opening()
    {

         onView(withId(R.id.genericTitleTxt)).check(matches(withText("Formatted View")))
    }
