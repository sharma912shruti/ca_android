...
class CreatePostTest {
    ...
    fun initPreWork(){
       ...
       // onView( withId( R.id.classmatesLayout)).perform( scrollTo(), click())
        ...
        onView(withId(R.id.createThreadImg)).perform(click())
        Thread.sleep(100)
    ...
    fun check_title_is_threads() {
        onView(withId(R.id.genericTitleTxt))
            .check(matches(ViewMatchers.withText("New Thread")))
    }

    @Test
    fun check_whether_able_to_type_subject_and_body()
    {
        /*threadSubjectEditTxt
        threadBodyEditTxt*/

        onView(withId(R.id.threadSubjectEditTxt)).perform( typeText("Test Subject"), ViewActions.closeSoftKeyboard())
        onView(withId(R.id.threadBodyEditTxt)).perform( typeText("Test message"), ViewActions.closeSoftKeyboard())
    }
