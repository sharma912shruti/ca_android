...
class AssignmentsTest {
    ...
    fun initPreWork(){
       ...
       // onView( withId( R.id.classmatesLayout)).perform( scrollTo(), click())
        onView( ViewMatchers.withText( "Assignments")).perform( scrollTo(), click())
    ...
    @Test
    fun check_title_is_assignments() {
        onView(withId(R.id.genericTitleTxt))
            .check(matches(ViewMatchers.withText("Assignments")))
    }

    @Test
    fun check_course_information_is_available_for_processing()
    {
        var activity = getCurrentActivity() as AssignmentsActivity
        Assert.assertNotNull(activity.courseID)

    }

    @Test
    fun check_assignments_list_have_list_non_null() {
        var activity = getCurrentActivity() as AssignmentsActivity
        Assert.assertTrue(activity.getAssignmentListSize() >= 0)
    }
