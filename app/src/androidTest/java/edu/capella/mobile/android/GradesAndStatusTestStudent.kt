...
class GradesAndStatusTestStudent {
    ...
    @Test
    fun check_title_is_grades_and_status() {
        onView(withId(R.id.genericTitleTxt))
            .check(matches(ViewMatchers.withText("Grades and Status")))
    }

    @Test
    fun check_course_information_is_available_for_processing()
    {
        var activity = getCurrentActivity() as GradesStatusActivity

        Assert.assertNotNull(activity.isStudent)
        Assert.assertNotNull(activity.course_Id)
        Assert.assertNotNull(activity.course_pk)

    }

    @Test
    fun check_assignment_grades_available()
    {
    ...
    @Test
    fun check_other_grades_available()
    {

        onView(withId(R.id.otherGradesPopperLayout)).check(matches(isDisplayed())).perform(
            click())
